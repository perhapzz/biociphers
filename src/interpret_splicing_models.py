...
from data_prep import read_splicing_data_interpret


def get_pc_pvalues(cancer_attributes, normal_attributes, feature_names, prefix):
    """
    ...
    # Write splicing junction name, attribution values and p-values to a file.
    ...
    header_val = "LSV-Names\tMedian-Attributions\tCorrected P-values (FDR <= 1%)"
    np.savetxt(prefix + "splicing_interpret_pvalues_abs.txt", feature_all, header=header_val, fmt="%s\t%s\t%s")
...
def get_splicing_attributions(data, dnn_model):
    """
    Get attributions for cancer and normal samples using splicing junctions.
    :param data: dict, data dict with normal and cancer samples and labels.
    :param dnn_model: str, path to dnn tensorflow model
    :return: list, attributions for normal and cancer class and splicing junction names
    """
    # Get normal and cancer indices
    normal_idx = 0
    cancer_idx = 1
    #normal_data_idx = np.where(data['labels'][4600:4900, normal_idx] == 1)[0]
    #cancer_data_idx = np.where(data['labels'][4600:4900, cancer_idx] == 1)[0]
    ...
    # In the case of normal attributions, cancer samples are the baseline and normal samples are the samples
    ...
    return all_attrs_normal, all_attrs_cancer, data["lsv_names"]
...
if __name__ == '__main__':
    ...
    # Get normalized data
    data = read_splicing_data_interpret(input_data_file=args.input_data_file)

    # Get splicing attributions
    all_attrs_normal, all_attrs_cancer, lsv_names = get_splicing_attributions(data, args.dnn_model)

    # Get p-values and store attributions
    get_pc_pvalues(all_attrs_cancer, all_attrs_normal, lsv_names, args.output_path)
